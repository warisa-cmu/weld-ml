...
class TFT(nn.Module):
    def __init__(self, config):
        ...
        self.time_varying_categoical_variables = config[
            "time_varying_categoical_variables"
        ]
        ...
        for i in range(self.static_variables):
            emb = nn.Embedding(
                config["static_embedding_vocab_sizes"][i], config["embedding_dim"]
            ).to(self.device)
        ...
        for i in range(self.time_varying_categoical_variables):
            emb = TimeDistributed(
                nn.Embedding(
                    config["time_varying_embedding_vocab_sizes"][i],
        ...
        for i in range(self.time_varying_real_variables_encoder):
            emb = TimeDistributed(
                nn.Linear(1, config["embedding_dim"]), batch_first=True
            ).to(self.device)
        ...
        self.static_enrichment = GatedResidualNetwork(
            ...
            self.hidden_size,
            self.dropout,
            config["embedding_dim"] * self.static_variables,
        )
        ...
        self.output_layer = TimeDistributed(
            nn.Linear(self.hidden_size, self.num_quantiles), batch_first=True
        )

    def init_hidden(self):
        return torch.zeros(
            self.lstm_layers, self.batch_size, self.hidden_size, device=self.device
        )
    ...
    def apply_embedding(self, x, static_embedding, apply_masking):
        ...
        if apply_masking:
            ...
            for i in range(self.time_varying_real_variables_decoder):
                emb = self.time_varying_linear_layers[
                    i + self.num_input_series_to_mask
            ...
            time_varying_real_vectors = []
            for i in range(self.time_varying_real_variables_encoder):
                emb = self.time_varying_linear_layers[i](
                    x[:, :, i].view(x.size(0), -1, 1)
                )
                time_varying_real_vectors.append(emb)
            time_varying_real_embedding = torch.cat(time_varying_real_vectors, dim=2)
        ...
        for i in range(self.time_varying_categoical_variables):
            emb = self.time_varying_embedding_layers[i](
                x[:, :, self.time_varying_real_variables_encoder + i]
                .view(x.size(0), -1, 1)
                .long()
            )
            time_varying_categoical_vectors.append(emb)
        time_varying_categoical_embedding = torch.cat(
            time_varying_categoical_vectors, dim=2
        )

        ##repeat static_embedding for all timesteps
        static_embedding = torch.cat(
            time_varying_categoical_embedding.size(1) * [static_embedding]
        )
        static_embedding = static_embedding.view(
            time_varying_categoical_embedding.size(0),
            time_varying_categoical_embedding.size(1),
            -1,
        )
        ...
        embeddings = torch.cat(
            [
                static_embedding,
                time_varying_categoical_embedding,
    ...
    def forward(self, x):
        ...
        pe = self.position_encoding(
            torch.zeros(self.seq_length, 1, embeddings_encoder.size(2)).to(self.device)
        ).to(self.device)
        ...
        attn_output = (
            self.post_attn_gate(attn_output) + attn_input[self.encode_length :, :, :]
        )
