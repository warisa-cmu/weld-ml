...
class MyModel_Min(nn.Module):
    def __init__(
        ...
        self.mlp_tab = nn.Sequential(
            ...
            nn.Linear(64, 32),
            nn.ReLU(),
            nn.Linear(32, ts_embedding_size),
        )

        comb_size = ts_embedding_size
    ...
    def forward(self, x_tab, x_ts, x_future):
        ...
        tab_out_flatten = tab_out.view(batch_size, -1)

        final = self.mlp_last(tab_out_flatten)
        return final, final
...
class MyModel_Attn(nn.Module):
    def __init__(
        ...
        self.mlp_tab = nn.Sequential(
            nn.Linear(
                tab_num_features * ts_embedding_size,
                lstm_num_layers * ts_embedding_size,
            ),
        )
